...
export class Diff {
  ...
  format(m2: ModJs | DateOptions | Date | string | number, str: string = "前") {
    ...
    const names = ["seconds", "minutes", "hours", "date", "month", "year"].reverse();
  ...
  */
  calculator(timeStamp: number) {
    ...
    const names = ["seconds", "minutes", "hours", "date", "month", "year"]
    const times = [1000, 60, 60, 24, 30, 12];
    if (this.config.exact) {
      const minIndex = names.indexOf(this.config.exactValue ?? 'minutes')
      times.forEach((item, index) => {
        const value = Math.floor(timeStamp / item) % (times[index + 1] ?? Number.MAX_SAFE_INTEGER);;
  ...
  processConfig(config: DiffConfig) {
    ...
    if (config.maxTimeStamp) {
      this.maxTime = isString(config.maxTimeStamp) ? Number.MAX_SAFE_INTEGER : config.maxTimeStamp;
    } else {
